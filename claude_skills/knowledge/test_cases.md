...
| 1.6 | ❌ | Sign up with empty name field | Validation error, form not submitted |
| 1.7 | ❌ | Sign up with invalid email format (e.g., "abc@") | Validation error shown |
| 1.8 | ❌ | Sign up with password less than minimum length | Validation error shown |
| 1.9 | ❌ | Sign up with empty fields (all blank) | Validation errors on all fields |
...
| 2.8 | ✅ | Click "Forgot Password?" link on login page | Navigates to /forgot-password page |
...
| # | Type | Test Case | Expected Result |
|---|------|-----------|-----------------|
| 3.1 | ✅ | Click verification link from signup email with valid token | Account verified, success message shown |
| 3.2 | ✅ | Request verification from Profile page ("Verify Account" button) | Verification email re-sent, success toast |
| 3.3 | ✅ | Verify and check Profile badge updates | Badge changes from "Not verified" to "Verified" |
| 3.4 | ❌ | Visit /verify with invalid/expired token | Error: "Invalid or expired token" |
| 3.5 | ❌ | Visit /verify with no token parameter | Error message shown |
| 3.6 | ❌ | Click verify link for already-verified account | Message: "Account is already verified" |
...
| # | Type | Test Case | Expected Result |
|---|------|-----------|-----------------|
| 3B.1 | ✅ | Enter valid email on /forgot-password | Success: "If an account with that email exists, a reset link has been sent" |
| 3B.2 | ✅ | Check inbox for reset email with branded template | Email received with "Reset Password" button (1hr expiry) |
| 3B.3 | ✅ | Enter non-existent email on /forgot-password | Same success message (email enumeration prevention) |
| 3B.4 | ❌ | Submit with empty email field | Validation error |
| 3B.5 | ❌ | Submit with invalid email format | Validation error |
...
| # | Type | Test Case | Expected Result |
|---|------|-----------|-----------------|
| 3C.1 | ✅ | Click reset link from email → set new password | Password reset, success message, auto-redirect to login |
| 3C.2 | ✅ | Login with new password after reset | Login successful |
| 3C.3 | ✅ | Check confirmation email received after password reset | "Password Changed" email in inbox |
| 3C.4 | ❌ | Visit /reset-password with expired token (>1hr) | Error: "Invalid or expired reset link" |
| 3C.5 | ❌ | Visit /reset-password with no token | Error: "Invalid Reset Link" page shown |
| 3C.6 | ❌ | Use same reset link twice | Error: "This reset link has already been used" |
| 3C.7 | ❌ | Set password less than 6 characters | Error: "Password must be at least 6 characters" |
| 3C.8 | ❌ | Passwords don't match (new vs confirm) | Error: "Passwords do not match" |

---

## 3D. Change Password (Profile)

| # | Type | Test Case | Expected Result |
|---|------|-----------|-----------------|
| 3D.1 | ✅ | Change password with correct old password | Password changed, success message, confirmation email sent |
| 3D.2 | ✅ | Login with new password after change | Login successful |
| 3D.3 | ❌ | Enter wrong old password | Error: "Current password is incorrect" |
| 3D.4 | ❌ | New password same as old password | Error: "New password cannot be the same" |
| 3D.5 | ❌ | New password less than 6 characters | Error: "Must be at least 6 characters" |
| 3D.6 | ❌ | New password and confirm don't match | Error: "Passwords do not match" (frontend validation) |
| 3D.7 | ✅ | Cancel change password form | Form collapses, fields reset |
...
| 4.7 | ✅ | See "Change Password" section on profile page | Change button visible, form expandable |
...
| 8.10 | ✅ | Ad stored in "Recently Viewed" (server-synced for logged-in users) | Ad recorded in DB via API |
...
| # | Type | Test Case | Expected Result |
|---|------|-----------|-----------------|
| 11.1 | ✅ | View 3 ads while logged in → go to homepage | "Recently Viewed" section shows those 3 ads (from DB) |
| 11.2 | ✅ | View same ad twice | Only 1 entry in recently viewed (upsert, no duplicates) |
| 11.3 | ✅ | View 25 ads | Only latest 20 stored (DB cap enforced) |
| 11.4 | ❌ | Not logged in → view ads → go to homepage | "Recently Viewed" section not shown (requires login) |
| 11.5 | ❌ | No ads viewed yet (logged in) | "Recently Viewed" section hidden on homepage |
| 11.6 | ✅ | View ads on device A, login on device B | Same recently viewed history visible (cross-device sync) |
...
| 16.5 | ✅ | Favorited ad has price reduced → get notification | "📉 Price Dropped!" notification with old/new price |
| 16.6 | ✅ | Favorited ad has price increased → get notification | "📈 Price Changed!" notification with old/new price |
| 16.7 | ✅ | Favorited ad has description/title updated → get notification | "🔔 Wishlist Item Updated!" notification listing changed fields |
| 16.8 | ✅ | Favorited ad has images changed → get notification | Notification mentions "images" in changed fields |
| 16.9 | ❌ | Access notifications when not logged in | Redirect or empty state |
